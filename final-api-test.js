...
// This will test auth, create user, then test all APIs with dummy data
...
async function createAndGetToken() {
  console.log('👤 Creating user and getting token...\n');
  ...
  const registerResult = await apiCall('/api/auth/register', 'POST', userData);
  
  if (registerResult.success) {
    console.log('✅ User registered successfully');
  } else {
    console.log('ℹ️  User might already exist or registration failed');
    console.log('   Error:', registerResult.data?.message);
  }
  ...
  if (loginResult.success) {
    ...
    for (const cred of commonCreds) {
      ...
      if (result.success) {
        console.log('✅ Login successful with common credentials');
...
// Test admission APIs with dummy data
async function testAdmissionAPIs(token) {
  ...
  if (listResult.success) {
    ...
    listResult.data.data.admissions.slice(0, 3).forEach((adm, index) => {
...
async function testPaymentAPIs(token) {
  ...
  if (listResult.success) {
    ...
    listResult.data.data.payments.slice(0, 3).forEach((payment, index) => {
  ...
  if (overdueResult.success) {
    ...
    if (overdueResult.data.data.overdueInstallments.length > 0) {
      ...
      overdueResult.data.data.overdueInstallments.slice(0, 3).forEach((inst, index) => {
...
async function testOtherAPIs(token) {
  ...
  const enquiryData = {
    ...
    source: "website",
    status: "NEW",
    note: "Test enquiry from API test"
  };
  
  const createEnquiryResult = await apiCall('/api/enquiries', 'POST', enquiryData, token);
  testResults.push(createEnquiryResult);
  
  if (createEnquiryResult.success) {
    console.log('✅ SUCCESS: Enquiry created');
    console.log(`   ID: ${createEnquiryResult.data.data.enquiry._id}`);
    console.log(`   Name: ${createEnquiryResult.data.data.enquiry.name}`);
    console.log(`   Course: ${createEnquiryResult.data.data.enquiry.course}`);
  } else {
    console.log('❌ FAILED: Could not create enquiry');
    console.log('   Error:', createEnquiryResult.data?.message || createEnquiryResult.error);
  }
...
async function runFinalAPITest() {
  ...
  if (!token) {
    ...
    console.log('   3. Auth endpoints are working');
    return;
  }
  ...
  console.log('   ✅ Full update APIs (no restrictions)');
