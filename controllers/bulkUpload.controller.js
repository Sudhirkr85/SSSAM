...
class BulkUploadController {
  uploadEnquiries = catchAsync(async (req, res) => {
    console.log('[DEBUG] uploadEnquiries API called', { userId: req.user?.id, userName: req.user?.name, userRole: req.user?.role });
    ...
    console.log('[DEBUG] File received:', { filename: req.file.originalname, size: req.file.size, mimetype: req.file.mimetype });
    ...
    console.log('[DEBUG] Parsed rows:', { rows: data.length });
